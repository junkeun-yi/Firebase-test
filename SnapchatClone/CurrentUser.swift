...
//        try! Auth.auth().signOut()
    ...
    func getReadPostIDs(completion: @escaping ([String]) -> Void) {
        ...
        var retrieved: [String: AnyObject] = [:]
        
        Database.database().reference().child(firPostsNode).observeSingleEvent(of: .value) { (snapshot) in
            let snapshotval = snapshot.value
            retrieved = snapshotval as! [String : AnyObject]
        }
    ...
    func addNewReadPost(postID: String) {
        Database.database().reference().child(firReadPostsNode).childByAutoId().setValue(postID)
    }
